import * as faceShape from "./src/utils/face_shape.js";
import * as eyeShape from "./src/utils/eye_shape.js";
import * as hairLines from "./src/utils/hair_lines.js";
import * as mouthShape from "./src/utils/mouth_shape.js";
const state = {
...
const generateFace = () => {
  ...
  state.eyeRightCountour = right.upper
    .slice(10, 90)
    .concat(right.lower.slice(10, 90).reverse());
  state.eyeLeftUpper = left.upper;
  state.eyeLeftLower = left.lower;
  state.eyeLeftCountour = left.upper
    .slice(10, 90)
    .concat(left.lower.slice(10, 90).reverse());
  state.distanceBetweenEyes = randomFromInterval(
    state.faceWidth / 4.5,
    state.faceWidth / 4
  );
  state.eyeHeightOffset = randomFromInterval(
    state.faceHeight / 8,
    state.faceHeight / 6
  );
  state.leftEyeOffsetX = randomFromInterval(
    -state.faceWidth / 20,
    state.faceWidth / 10
  );
  state.leftEyeOffsetY = randomFromInterval(
    -state.faceHeight / 50,
    state.faceHeight / 50
  );
  state.rightEyeOffsetX = randomFromInterval(
    -state.faceWidth / 20,
    state.faceWidth / 10
  );
  state.rightEyeOffsetY = randomFromInterval(
    -state.faceHeight / 50,
    state.faceHeight / 50
  );
  state.leftEyeCenter = left.center[0];
  state.rightEyeCenter = right.center[0];
  state.leftPupilShiftX = randomFromInterval(
    -state.faceWidth / 20,
    state.faceWidth / 20
  );
  ...
  state.hairs = [];
  if (Math.random() > 0.3) {
    state.hairs = hairLines.generateHairLines0(
      state.computedFacePoints,
      numHairLines[0] * 1 + 10
    );
  }
  if (Math.random() > 0.3) {
    state.hairs = state.hairs.concat(
  ...
  state.rightNoseCenterX = randomFromInterval(
    state.faceWidth / 18,
    state.faceWidth / 12
  );
  state.rightNoseCenterY = randomFromInterval(0, state.faceHeight / 5);
  state.leftNoseCenterX = randomFromInterval(
    -state.faceWidth / 18,
    -state.faceWidth / 12
  );
  state.leftNoseCenterY =
    state.rightNoseCenterY +
    randomFromInterval(-state.faceHeight / 30, state.faceHeight / 20);
  if (Math.random() > 0.1) {
  ...
  if (choice == 0) {
    state.mouthPoints = mouthShape.generateMouthShape0(
      state.computedFacePoints,
      state.faceHeight,
      state.faceWidth
    );
  } else if (choice == 1) {
    state.mouthPoints = mouthShape.generateMouthShape1(
      state.computedFacePoints,
      state.faceHeight,
      state.faceWidth
    );
  } else {
    state.mouthPoints = mouthShape.generateMouthShape2(
      state.computedFacePoints,
      state.faceHeight,
      state.faceWidth
    );
  ...
  <linearGradient id="rainbowGradient" x1="0%" y1="0%" x2="100%" y2="0%">
    <stop
      ...
      style="
        ${
          "stop-color: " +
          state.hairColors[Math.floor(Math.random() * 10)] +
          ";  stop-opacity: 1"
        }
      "
    />
    <stop
      ...
      style="
        ${
          "stop-color: " +
          state.hairColors[
            Math.floor(Math.random() * state.hairColors.length)
          ] +
          ";  stop-opacity: 1"
        }
      "
  ...
  fill="
    ${
      state.backgroundColors[
        Math.floor(Math.random() * state.backgroundColors.length)
      ]
    }
  "
/>
